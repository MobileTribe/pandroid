...
TIP: link:{github}{demoJava}main/event/EventFragment.java[This example] is available in the link:{github}{demoDir}[demo project]
...
----
pandroid{
    library('libName') { //Regex matching declared configurator that will bring the dependency
        libParam1 'param1'
        libParam2 'param2'
    }
}
...
The plugin is using configurators in order to extract the librairies configurations from Pandroid.
It allows you to modify the gradle's configuration and the manifest of your app when the plugin is applied.

To see default configurators, take a look at this link:{github}{libDir}pandroid.gradle[pandroid.gradle].
    ...
    configurators{
        ...
        <application>
            <meta-data
                ...
                android:value="''' + config.libParam1 + '''" />
        </application>
    '''
    }
}
...
In Pandroid, an Opener represent the argument of an Activity or a Fragment. In order to work an opener needs an OpenerEventReceiver.
When you need to open an Activity or a Fragment you just send an opener in the event bus. The OpenerEventReceiver in charge
will catch the event and open the Activity/Fragment with the opener as argument.
Then you will be able to access the opener in the PandroidActivity/PandroidFragment with the field opener.
The main idea behind Opener is to normalize navigation in the application and remove code responsive of it from your views.

NOTE: See link:#_PandroidActivity[PandroidActivity]/link:#_pandroidfragment[PandroidFragment] part for more details.
...
To open activity you are encouraged to use the ActivityOpener and ActivityEventReceiver. To make it work you have to:

- register an ActivityEventReceiver in you PandroidApplication or your current PandroidActivity.

[source,java]
----
include::{demoJava}DemoApplication.java[tags=ActivityOpener]
----

- Then you will be able to send the opener through the event bus.

[source,java]
----
include::{demoJava}main/NavigationLeftFragment.java[tags=ActivityOpener]
----

- And access your param in the opened Activity

[source,java]
----
include::{demoJava}main/opener/OpenerActivity.java[tags=ActivityOpener]
----


NOTE: you are please to look at the link:{github}{demoJava}main/opener[opener part] in the demo to see it in action
...
By default PandroidLifecycleDelegate is created in your PandroidApplication.
...
=== Auto bind Lifecycle Delegate

If you need to associate your custom Lifecycle Delegate automatically you can use the annotation @BindLifeCycleDelegate in PandroidActivity/Fragment

Example :

[source,java]
----
include::{demoJava}main/mvp/PresenterFragment.java[tags=PandroidBindLifeCycleDelegateAnnotationUseCase]
----

NOTE: If your delegate is not initialized or inject before the onCreate then the auto binder will try to create an
instance by himself

WARNING: The class initialization must be done in the createDelegate() or before its call. If you do it after that your class will be ignored
...
pandroid
{
    dependencies{
        library ("crashlytics"){
            apiKey "YOUR_CRASHLYTICS_API_KEY"
        }
    }
}
...
That's it ! The crashlytics logger will be added automatically. By default it is is configured to be activated when the BuildConfig.DEBUG is false
If needed you can override this behaviour

=== RxAndroid

RxAndroid is an optional library in pandroid.
...
Once enable you will be able to use Rx version of some manager (RxCryptoManager, RxConfigurationManager, ...).
RxPandroidCall will be available automatically with retrofit and you will be able to use RxPandroidActivity/RxPandroidFragment
in order to access useful methods to register you Observable on the app lifecycle.

[source,java]
----
include::{demoJava}main/rx/RxFragment.java[tags=RxAndroid]
----


.RxWrapper

@RxWrapper annotation can be used to generate a rx version of you class.
[source,java]
----
include::{annotationsJava}RxWrapper.java[tags=RxWrapper]
----
...
----
//ReviewManager.java
include::{demoJava}globals/review/ReviewManager.java[tags=RxWrapper]

//DemoModule.java
include::{demoJava}DemoModule.java[tags=RxWrapper]

//RxFragment.java
include::{demoJava}main/rx/RxFragment.java[tags=RxWrapper]
----


=== Android DataBinding

Pandroid provided an annotation to help the use of Android Data Binding in your project.
This feature has been disabled for now but should returned soon. If you want to check its old implementation see branch feature/databinding


=== MVP

A good application, is an appliction well structured. To help the developers Pandroid provide a Presenter class
to use the famous pattern MVP. The Presenter is bound to the application's lifecycle.

NOTE: see link:{github}{demoJava}main/mvp[the demo] to see it in action

=== Templates

To help you, Pandroid provides templates for Android Studio.
In order to install them, you just have to apply link:https://github.com/MobileTribe/android-templates-gradle-plugin['com.leroymerlin.android-templates'] plugin on your project. Pandroid will add and update the templates automatically.

To access the templates, restart once the IDE and a Pandroid entry will be available in the *"new"* menu (right click on your package > new, or cmd + n)
...
pandroid{
    secureConfigFields{
        ...
        //apply on applicationVariant that contrains the name
        prod { //proDebug prodRelease
...
[source,java]
----
include::{javaJava}future/ActionDelegate.java[]
----
...
Tip : See https://github.com/bumptech/glide to use Glide
...
A easiest implementation is available for RecyclerView

[source,java]
----
include::{demoJava}main/list/SimpleRecyclerViewFragment.java[tags=PandroidAdapter]
----
...
[source,java]
----
include::{demoJava}main/list/KotlinRVFragment.kt[tags=KotlinDSL]
----
...
If you need a custom behaviour in your layout inflater you can override *getLayoutInflaterFactories*
in PandroidApplication to provide your own factories. If needed you can implement *PandroidFactoryProvider*
in your Activity to add a local factory.

By default Pandroid is configure to add a ViewCompatFactory if you enable it in the extension.
See Pandroid *PandroidCompat* for details
...
PandroidCompat(TextView/EditText/...) are available to support Vector Drawable on pre lollipop devices.
To enable this support use pandroid extension
...
[source,xml]
----
include::{demoRes}layout/fragment_animation.xml[tags=vectorCompat]
----
