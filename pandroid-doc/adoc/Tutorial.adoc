...
If you need to associate your custom Lifecycle Delegate automatically so just use the annotation :

[source,java]
----
include::{annotationsJava}BindLifeCycleDelegate.java[tags=PandroidBindLifeCycleDelegateAnnotation]
----
...
NOTE: if the tool is not initialized by dagger or manually when the auto binder create an
instance if there is an empty constructor

WARNING: To initialize after declaration must put your statement in the createDelegate() method
overloading
